  ...
  render() {
    return (
      <div>
        ...
        <h5>World</h5>
          <img src="https://source.unsplash.com/300x200/?world" className="img-fluid" alt="Wild Landscape" style={{ width: "400px", height: "230px" }} />
        </div>
        {
          this.state.article.map((value, i) => {
            return this.state.loading ? "" : <SmallNewsCard url = {value.link} key={value.link} src = "https://source.unsplash.com/300x200/?world,history" date = {value.pubDate} title = {value.title} image= {value.image_url}/>
          })
        }
      </div>
    )
